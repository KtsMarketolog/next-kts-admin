...
type HtmlRow = {
  id: string; audience: PersonalDashboardAudience; original_name: string; file_size: string; sha256: string; created_at: string;
...
const IMPORT_MESSAGES: Record<PersonalDashboardImportStatus, string> = {
  ...
  unknown: 'Не найден активный менеджер по развитию или сопровождению с email этого снимка',
...
const HTML_SELECT = `v.id::text, v.audience, v.original_name, v.file_size::text, v.sha256, v.created_at::text,
...
function checkedAudience(value: PersonalDashboardAudience): PersonalDashboardAudience {
  const audience = parsePersonalDashboardAudience(value);
  if (!audience) throw new PersonalDashboardError('NOT_FOUND', 'Группа личного дашборда не найдена');
  return audience;
}
...
function mapHtml(row: HtmlRow, activeId: string | null): PersonalDashboardHtmlVersion {
  return {
    id: Number(row.id), audience: row.audience, originalName: row.original_name, fileSize: Number(row.file_size), sha256: row.sha256,
...
export function resolvePersonalDashboardManager(managers: PersonalDashboardManagerBinding[], emailHash: string) {
  const matches = managers.filter((manager) => manager.isActive && getPersonalDashboardAudience(manager.role) !== null
...
async function authorizeManagerAccount(client: PoolClient, managerId: number) {
  ...
  const audience = getPersonalDashboardAudience(manager?.role);
  if (!manager?.is_active || !audience) {
    throw new PersonalDashboardError('NOT_FOUND', 'Личный дашборд недоступен');
  }
  return { manager, managers, audience };
}
async function accountSnapshotBinding(client: PoolClient, managerId: number) {
  const { manager, managers, audience } = await authorizeManagerAccount(client, managerId);
  // Access to the group's shared published HTML requires a valid manager account,
  // not a working snapshot email. Snapshot identity is a separate, stricter condition.
  if (!manager.email.trim()) return { audience, status: 'missing_email' as const, emailHash: null };
  ...
  if (binding.managerId !== managerId) return { audience, status: 'ambiguous_email' as const, emailHash: null };
  return { audience, status: 'matched' as const, emailHash };
}
...
export async function getPersonalDashboardStatus(managerId: number): Promise<PersonalDashboardStatus> {
  ...
  return withTransaction(async (client) => {
    const binding = await accountSnapshotBinding(client, managerId);
    if (binding.status !== 'matched') return { audience: binding.audience, snapshot: null, history: [], bindingStatus: binding.status };
    ...
    return { audience: binding.audience, snapshot: history.find((item) => item.status === 'active') ?? null, history, bindingStatus: 'matched' };
  });
}
...
export async function getPersonalDashboardHtml(id?: number, preview = false, requestedAudience: PersonalDashboardAudience = 'development') {
  if (id !== undefined) positiveId(id);
  const audience = checkedAudience(requestedAudience);
  await ensureSiteSchema();
  const result = await query<HtmlRow & { html_content: string; active_version_id: string | null }>(`select ${HTML_SELECT}, v.html_content, st.active_version_id::text
    ...
    where st.audience=$3 and v.audience=st.audience and v.id=coalesce($1::bigint,st.active_version_id)
      and ($2::boolean or (v.id=st.active_version_id and v.first_published_at is not null))`, [id ?? null, preview, audience]);
  const row = result.rows[0];
...
export async function createPersonalDashboardHtml(input: {
  originalName: string; htmlContent: string; fileSize: number; sha256: string; actorId: string; audience?: PersonalDashboardAudience;
}) {
  const audience = checkedAudience(input.audience === undefined ? 'development' : input.audience);
  ...
  return withTransaction(async (client) => {
    ...
    const quota = await client.query<{ count: string; bytes: string }>(`select count(*)::text as count, coalesce(sum(file_size),0)::text as bytes from personal_dashboard_html_versions where audience=$1`, [audience]);
    ...
    const result = await client.query<HtmlRow>(`insert into personal_dashboard_html_versions
      (original_name,html_content,file_size,sha256,uploaded_by,audience) values ($1,$2,$3,$4,$5,$6)
      returning id::text, audience, original_name, file_size::text, sha256, created_at::text, uploaded_by, first_published_at::text`,
    [personalDashboardSafeFilename(input.originalName), input.htmlContent, size, input.sha256, uploadedBy, audience]);
    return mapHtml(result.rows[0], null);
  });
}

export async function activatePersonalDashboardHtml(input: { versionId: number; expectedActiveVersionId: number | null; actorId: string; audience?: PersonalDashboardAudience }) {
  const audience = checkedAudience(input.audience === undefined ? 'development' : input.audience);
  ...
  return withTransaction(async (client) => {
    await client.query(`select pg_advisory_xact_lock(hashtext($1))`, [`kts-personal-dashboard-html:${audience}`]);
    const stateResult = await client.query<HtmlStateRow>(`select active_version_id::text, previous_version_id::text from personal_dashboard_html_state where audience=$1 for update`, [audience]);
    ...
    if (Number(state.active_version_id) !== input.versionId) {
      await client.query(`update personal_dashboard_html_versions set first_published_at=coalesce(first_published_at,now()),
        first_published_by=coalesce(first_published_by,$2) where id=$1 and audience=$3`, [input.versionId, publishedBy, audience]);
  ...
  return withTransaction(async (client) => {
    ...
    return {
      groups: (['development', 'support'] as const).map((audience) => {
        const groupState = state.rows.find((row) => row.audience === audience);
        const activeId = groupState?.active_version_id ?? null;
        return {
          audience,
