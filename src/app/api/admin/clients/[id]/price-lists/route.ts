...
import { getClientCompanyPriceLists } from '@/shared/lib/db';
...
export async function GET(_request: Request, context: Context) {
  ...
  try {
    const priceLists = await getClientCompanyPriceLists(clientId, { session });
    return Response.json({ priceLists });
  } catch (error) {
    return Response.json(
      { error: error instanceof Error ? error.message : 'Не удалось загрузить прайсы клиента' },
      { status: 400 },
    );
  }
}
