...
import { getClientPriceRequestsForAdmin } from '@/shared/lib/db';
...
export async function GET(_request: Request, context: Context) {
  ...
  try {
    const requests = await getClientPriceRequestsForAdmin(clientId, session);
    return Response.json({ requests });
  } catch (error) {
    return Response.json(
      { error: error instanceof Error ? error.message : 'Не удалось загрузить заявки клиента' },
      { status: 400 },
    );
  }
}
