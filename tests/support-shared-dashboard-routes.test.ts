...
function routes(role: Role = 'support_manager', options: {inactive?: boolean; currentRole?: string; limit?: boolean; noSnapshot?: boolean; expired?: boolean} = {}) {
  ...
  const overview = {htmlVersions: [{id: 22, audience: 'support', originalName: 'shared.html'}], activeHtmlVersionId: 22, previousHtmlVersionId: null, snapshot, history: snapshot ? [snapshot] : []};
  const repo = {
    getSupportSharedDashboardOverview: observed('overview', () => overview),
    getSupportSharedDashboardHtml: observed('html', (id, preview) => id === 22 || (preview && id === 23) ? {id, htmlContent: fixture} : null),
